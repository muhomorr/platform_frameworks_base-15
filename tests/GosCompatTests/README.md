...
Generally, the device should be unlocked and on user 0 while the tests are running. The tests can
be run on user builds both via `atest` and using the standalone UI in `GosCompatCheckApp`.
