...
public class LockSettingsServiceTests extends BaseLockSettingsServiceTests {
    ...
    public void testUserCredentialCache(LockDomain domain) throws Exception {
        final int userId = PRIMARY_USER_ID;

        // The initial getCredentialType() should return NONE and set the cache entry.
        assertEquals(CREDENTIAL_TYPE_UNKNOWN, mService.queryServiceSideCredentialTypeCache(userId, domain));
        assertEquals(CREDENTIAL_TYPE_NONE, mService.getCredentialType(userId, domain));
        assertEquals(CREDENTIAL_TYPE_NONE, mService.queryServiceSideCredentialTypeCache(userId, domain));

        // Set a PIN.
        assertTrue(mService.setLockCredential(newPin("1234"), nonePassword(), domain, userId));
        ...
        assertEquals(CREDENTIAL_TYPE_PIN, mService.queryServiceSideCredentialTypeCache(userId, domain));
    ...
    public void testUserCredentialCache_disabled() throws Exception {
        ...
        assertEquals(CREDENTIAL_TYPE_UNKNOWN, mService.queryServiceSideCredentialTypeCache(userId, Primary));
        assertEquals(CREDENTIAL_TYPE_NONE, mService.getCredentialType(userId, Primary));
        assertEquals(CREDENTIAL_TYPE_UNKNOWN, mService.queryServiceSideCredentialTypeCache(userId, Primary));
        assertTrue(mService.setLockCredential(newPin("1234"), nonePassword(), Primary, userId));
        assertEquals(CREDENTIAL_TYPE_UNKNOWN, mService.queryServiceSideCredentialTypeCache(userId, Primary));
