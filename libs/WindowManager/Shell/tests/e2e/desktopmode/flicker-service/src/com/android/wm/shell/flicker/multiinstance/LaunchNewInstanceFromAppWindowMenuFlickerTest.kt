...
/** Test for opening new window from the app menu in Desktop Mode. */
@RequiresDesktopDevice
@Postsubmit
@RunWith(Parameterized::class)
@Parameterized.UseParametersRunnerFactory(FlickerParametersRunnerFactory::class)
class LaunchNewInstanceFromAppWindowMenuFlickerTest(flicker: FlickerTest) :
    DesktopModeBaseTest(flicker) {
    inner class LaunchNewWindowFromAppWindowScenario :
        LaunchNewInstanceFromAppWindowMenu(flicker.scenario.startRotation)

    @Rule
    @JvmField
    val testSetupRule = Utils.testSetupRule(NavBar.MODE_GESTURAL, flicker.scenario.startRotation)
    val scenario = LaunchNewWindowFromAppWindowScenario()

    private val browserApp = scenario.testApp
