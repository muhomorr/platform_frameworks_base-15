...
@DslMarker
annotation class CompatUIHandlerTestTagMarker

// Base class for the CompatUIHandler interfaces Test Context.
@CompatUIHandlerTestTagMarker
open class CompatUIHandlerTestContext(
    private val testSubjectFactory: () -> CompatUIHandler
) {
    ...
    fun compatUIInfo(
        builder: CompatUIInfoInputBuilder.() -> Unit
    ): CompatUIInfo {
        val compatUIInfoObj = CompatUIInfoInputBuilder()
        compatUIInfoObj.builder()
        return compatUIInfoObj.build().apply {
            compatUIInfo = this
        }
    }
