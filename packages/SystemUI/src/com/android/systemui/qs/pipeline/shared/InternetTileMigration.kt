...
@SysUISingleton
class InternetTileMigration
@Inject
constructor(private val connectivityConstants: ConnectivityConstants) {
    /**
     ...
     * @param isMainUser whether the migration is for the main user
    ...
    fun migrateInternetTile(
        ...
        if (QsSplitInternetTile.isEnabled) {
            return migrateInternetTileToWifiAndCell(
                currentTiles,
                largeTiles,
                isMainUser,
                logScenario,
     ...
     *   with two small tiles, visually. In some cases, the cellular tile is not available: either
     *   mobile data is not available, or the user is not the mainUser. In that case, we replace a
     *   large `internet` tile with a large `wifi` tile.
    ...
    private fun migrateInternetTileToWifiAndCell(
        ...
        val dataTileEnabled = connectivityConstants.hasDataCapabilities && isMainUser
        if (internetTileSpec in largeTiles && !dataTileEnabled) {
            mutableCurrentTiles.add(internetPosition, wifiTileSpec)
            logScenario(InternetTileMigrationScenario.LARGE_INTERNET_TO_LARGE_WIFI)
            return mutableCurrentTiles to largeTiles - internetTileSpec + wifiTileSpec
        } else {
    ...
    companion object {
         ...
         *
         * If [QsSplitInternetTile.isEnabled] is `true`, instances of `internet` will be converted
         * to `wifi`.
         *
