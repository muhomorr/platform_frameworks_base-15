...
import com.android.systemui.surfaceeffects.compose.modifier.RippleEffectNode
import com.android.systemui.surfaceeffects.compose.modifier.RippleEffectNodeElement
import com.android.systemui.surfaceeffects.compose.modifier.rippleCircleEffect
...
class RippleEffectImplTest : SysuiTestCase() {
    ...
    fun rippleProgress_reachesIntermediateAndFinalValues_afterDuration() {
        ...
        assertThat(node.rawProgress.floatValue).isWithin(PROGRESS_TOLERANCE).of(0.33F)

        composeTestRule.mainClock.advanceTimeBy(config.duration * 2 / 3)
        assertThat(node.rawProgress.floatValue).isWithin(PROGRESS_TOLERANCE).of(1F)
    }
    ...
    fun triggerKeyChanges_rippleEffectReplays() {
        ...
        assertThat(node.rawProgress.floatValue).isWithin(PROGRESS_TOLERANCE).of(0F)
        composeTestRule.mainClock.advanceTimeBy(config.duration / 3)
        assertThat(node.rawProgress.floatValue).isWithin(PROGRESS_TOLERANCE).of(0.33F)
        triggerKey += 1
        composeTestRule.mainClock.advanceTimeBy(config.duration / 3)
