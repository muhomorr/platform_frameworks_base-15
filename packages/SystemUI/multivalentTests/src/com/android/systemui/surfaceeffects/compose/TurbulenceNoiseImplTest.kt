...
import com.android.systemui.surfaceeffects.compose.modifier.TurbulenceNoiseNode
import com.android.systemui.surfaceeffects.compose.modifier.TurbulenceNoiseNodeElement
import com.android.systemui.surfaceeffects.compose.modifier.simplexNoiseEffect
...
class TurbulenceNoiseImplTest : SysuiTestCase() {
    ...
    fun progressAnimatable_autoStopWithMaxDuration() {
        ...
        assertThat(node.rawProgress.floatValue).isEqualTo(0F)
        assertThat(node.fadingProgress.floatValue).isEqualTo(0F)

        isEnabled = true
        composeTestRule.mainClock.advanceTimeBy(config.fadeInDuration.toLong())
        assertThat(node.rawProgress.floatValue).isWithin(PROGRESS_TOLERANCE).of(0.25F)
        assertThat(node.fadingProgress.floatValue).isWithin(PROGRESS_TOLERANCE).of(1F)

        composeTestRule.mainClock.advanceTimeBy(
            (config.maxDuration / 2 - config.fadeInDuration).toLong()
        )
        assertThat(node.rawProgress.floatValue).isWithin(PROGRESS_TOLERANCE).of(0.5F)
        assertThat(node.fadingProgress.floatValue).isWithin(PROGRESS_TOLERANCE).of(1F)

        composeTestRule.mainClock.advanceTimeBy((config.maxDuration * 0.375).toLong())
        assertThat(node.rawProgress.floatValue).isWithin(PROGRESS_TOLERANCE).of(0.875F)
        assertThat(node.fadingProgress.floatValue).isWithin(PROGRESS_TOLERANCE).of(1F)
    }
    ...
    fun progressAnimatable_manualFadeOut() {
        ...
        assertThat(node.rawProgress.floatValue).isWithin(PROGRESS_TOLERANCE).of(0.75f)
        assertThat(node.fadingProgress.floatValue).isWithin(PROGRESS_TOLERANCE).of(0f)
    }
